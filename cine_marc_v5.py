...
def render_card(film, prefix="c"):
    ...
    # Ligne 1 : [🔖 À voir 3/4] [👁 VU 1/4]
    col_av, col_vu = st.columns([3, 1])
    with col_av:
        if deja_avoir:
            if st.button("↩️ À voir", key=f"{prefix}_rav_{fid}", use_container_width=True):
                avoir_retirer(fid); st.rerun()
        elif not deja_vu:
            if st.button("🔖 À voir", key=f"{prefix}_av_{fid}", use_container_width=True):
