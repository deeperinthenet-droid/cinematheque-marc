...
def render_card(film, prefix="c"):
    ...
    col_pt, col_det, col_vu = st.columns([1, 2, 1])
    with col_pt:
        if deja_plus_tard:
...
#  TABS PRINCIPAUX
# ─────────────────────────────────────────────
tab_browse, tab_search, tab_plus_tard, tab_vus = st.tabs([
    "🎬  Catalogue",
    "🔍  Recherche",
    "🕐  Plus tard",
    "✅  Vus",
...
with tab_browse:
    ...
    if films:
        ...
        films_affiches = films if afficher_vus else [f for f in films if str(f["id"]) not in vus and str(f["id"]) not in avoirs and str(f["id"]) not in plus_tard]
...
with tab_plus_tard:
    ...
    st.markdown('<p style="color:#666;font-size:.85rem;margin-bottom:20px;">Films mis de côté — pas ce mois-ci.</p>', unsafe_allow_html=True)
    ...
    if not plus_tard_data:
        ...
        <div style="text-align:center;padding:4rem 0;">
            ...
            <p style="color:#555;font-size:.88rem;">Depuis le catalogue, cliquez sur 🕐 sous une affiche.</p>
        </div>""", unsafe_allow_html=True)
    else:
        ...
        for i in range(0, len(ids_list), 6):
            ...
            for j, (fid, info) in enumerate(ids_list[i:i+6]):
                with cols[j]:
                    ...
                    <div class="nf-card-container">
                        <div class="nf-card-img-wrap">
                            ...
                            <div class="nf-card-badges">
                                <span class="nf-badge-plus-tard">🕐</span>
                            </div>
                        </div>
                    </div>""", unsafe_allow_html=True)
                    ...
                    with bp1:
                        if st.button("✅ VU", key=f"pt_vu_{fid}", use_container_width=True):
                            vu_ajouter(int(fid), titre, annee); plus_tard_retirer(int(fid)); st.rerun()
                    with bp2:
                        if st.button("🗑", key=f"pt_del_{fid}", use_container_width=True):
