...
.nf-badge-plus-tard { background:#f5a623; color:#000; font-size:.52rem; font-weight:700; padding:2px 5px; border-radius:2px; display:inline-block; }
...
def _init_cache():
    """Charge VU, AVOIR et PLUS_TARD depuis GitHub une seule fois par session."""
    if "vu_data" not in st.session_state or "avoir_data" not in st.session_state or "plus_tard_data" not in st.session_state:
        vu, vu_sha     = _gh_read("films_vus.json")
        av, av_sha     = _gh_read("films_avoir.json")
        pt, pt_sha     = _gh_read("films_plus_tard.json")
        st.session_state.vu_data         = vu
        st.session_state.vu_sha          = vu_sha
        st.session_state.avoir_data      = av
        st.session_state.avoir_sha       = av_sha
        st.session_state.plus_tard_data  = pt
        st.session_state.plus_tard_sha   = pt_sha
...
# ── PLUS TARD ──
def plus_tard_charger():
    _init_cache()
    return st.session_state.plus_tard_data

def plus_tard_ajouter(fid, titre, annee):
    _init_cache()
    st.session_state.plus_tard_data[str(fid)] = {"titre": titre, "annee": annee, "date": datetime.now().strftime("%d/%m/%Y")}
    _gh_write_async("films_plus_tard.json", st.session_state.plus_tard_data, st.session_state.plus_tard_sha)

def plus_tard_retirer(fid):
    _init_cache()
    st.session_state.plus_tard_data.pop(str(fid), None)
    _gh_write_async("films_plus_tard.json", st.session_state.plus_tard_data, st.session_state.plus_tard_sha)

def plus_tard_ids():
    _init_cache()
    return set(st.session_state.plus_tard_data.keys())
...
for k, v in [
    ...
    ("hero_idx", 0), ("scroll_to_film_id", None),
]:
...
def render_card(film, prefix="c"):
    ...
    deja_plus_tard = str(fid) in plus_tard_ids()
    ...
    badge_plus_tard = '<span class="nf-badge-plus-tard">🕐 PLUS TARD</span>' if deja_plus_tard else ""

    # Scroll vers cette carte si on revient du détail
    should_scroll = st.session_state.get("scroll_to_film_id") == fid
    if should_scroll:
        st.session_state.scroll_to_film_id = None
    ...
    st.markdown(f"""
    <div id="film-{fid}" class="nf-card-container">
        <div class="nf-card-img-wrap">
            {img_content}
            <div class="nf-card-play">&#9654;</div>
            <div class="nf-card-badges">{badge_vu}{badge_avoir}{badge_plus_tard}</div>
    ...
    # Scroll via composant HTML dédié (plus fiable que inline script)
    if should_scroll:
        st.components.v1.html(f"""
        <script>
            (function() {{
                function doScroll() {{
                    var el = window.parent.document.getElementById('film-{fid}');
                    if (el) {{
    ...
    # Ligne 1 : [🕐 1/4] [vide 1/2] [👁 1/4]
    col_pt, col_mid, col_vu = st.columns([1, 2, 1])
    with col_pt:
        if deja_plus_tard:
            if st.button("↩️", key=f"{prefix}_rpt_{fid}", use_container_width=True):
                plus_tard_retirer(fid); st.rerun()
        elif not deja_vu:
            if st.button("🕐", key=f"{prefix}_pt_{fid}", use_container_width=True):
                plus_tard_ajouter(fid, titre, annee); st.rerun()
    with col_mid:
        st.empty()
    with col_vu:
        if deja_vu:
            if st.button("✅", key=f"{prefix}_vu_{fid}", use_container_width=True):
                vu_retirer(fid); st.rerun()
        else:
            if st.button("👁", key=f"{prefix}_vu_{fid}", use_container_width=True):
                vu_ajouter(fid, titre, annee)
                avoir_retirer(fid); st.rerun()
    # Ligne 2 : ▶ Détails (pleine largeur)
    if st.button("▶ Détails", key=f"{prefix}_det_{fid}", use_container_width=True):
        st.session_state.film_detail = fid
        st.session_state.scroll_to_film_id = fid
        st.rerun()

def render_detail(film_id):
    # ── Bouton retour EN PREMIER — avant tout appel API ──
    # Cela garantit que Streamlit commence le rendu depuis le haut
    st.markdown('''<div class="nf-retour-btn">''', unsafe_allow_html=True)
    if st.button("← Retour à la sélection", key=f"md_back_{film_id}"):
        ...
        st.session_state.scroll_to_film_id = film_id
        st.rerun()
    ...
    with col_txt:
        ...
        deja_plus_tard = str(film_id) in plus_tard_ids()
        st.markdown('<p class="nf-lbl">Mettre de côté</p>', unsafe_allow_html=True)
        if deja_plus_tard:
            if st.button("↩️ Retirer de Plus tard", key=f"md_rpt_{film_id}"):
                plus_tard_retirer(film_id); st.rerun()
        else:
            if st.button("🕐 Plus tard — pas ce mois-ci", key=f"md_pt_{film_id}"):
                plus_tard_ajouter(film_id, titre, annee); st.rerun()
...
# ─────────────────────────────────────────────
#  PAGE DÉTAIL — INTERCEPTION AVANT TOUT LE RESTE
# ─────────────────────────────────────────────
...
#  TABS PRINCIPAUX
# ─────────────────────────────────────────────
tab_browse, tab_search, tab_avoir, tab_plus_tard, tab_vus = st.tabs([
    "🎬  Catalogue",
    "🔍  Recherche",
    "🔖  À Voir",
    "🕐  Plus tard",
...
with tab_browse:
    ...
    if films:
        ...
        plus_tard = plus_tard_ids()
        films_affiches = films if afficher_vus else [f for f in films if str(f["id"]) not in vus and str(f["id"]) not in plus_tard]
...
with tab_plus_tard:
    ...
    plus_tard_data = plus_tard_charger()

    st.markdown('<p style="font-family:\'Bebas Neue\',sans-serif;font-size:2rem;color:#fff;margin-bottom:16px;">🕐 À voir plus tard</p>', unsafe_allow_html=True)
    st.markdown('<p style="color:#666;font-size:.85rem;margin-bottom:20px;">Films mis de côté — pas ce mois-ci, mais dans ta liste pour plus tard.</p>', unsafe_allow_html=True)

    if not plus_tard_data:
        st.markdown("""
        <div style="text-align:center;padding:4rem 0;">
            <div style="font-size:3rem;margin-bottom:1rem;">🕐</div>
            <p style="font-family:'Bebas Neue',sans-serif;font-size:1.6rem;color:#fff;margin-bottom:.6rem;">Rien de mis de côté</p>
            <p style="color:#555;font-size:.88rem;">Depuis le catalogue ou la fiche d'un film, cliquez sur "🕐 Plus tard".</p>
        </div>""", unsafe_allow_html=True)
    else:
        st.metric("Films mis de côté", len(plus_tard_data))
        st.markdown('<div style="height:16px;"></div>', unsafe_allow_html=True)

        ids_list = list(plus_tard_data.items())
        for i in range(0, len(ids_list), 6):
            cols = st.columns(6, gap="small")
            for j, (fid, info) in enumerate(ids_list[i:i+6]):
                with cols[j]:
                    ...
                    img_src = f"https://image.tmdb.org/t/p/w300{poster_path}" if poster_path else ""
                    img_html = (f'<img src="{img_src}" alt="{titre}" loading="lazy">'
                                if img_src else '<div class="nf-card-img-ph">🎬</div>')
                    ...
                    <div class="nf-card-container">
                        <div class="nf-card-img-wrap">
                            {img_html}
                            <div class="nf-card-badges">
                                <span class="nf-badge-plus-tard">🕐 PLUS TARD</span>
                            </div>
                            <div class="nf-card-overlay">
                                <div class="nf-card-title">{titre}</div>
                                <div class="nf-card-meta">
                    ...
                    if st.button("▶ Détails", key=f"pt_det_{fid}", use_container_width=True):
                        st.session_state.film_detail = int(fid); st.rerun()

                    bp1, bp2 = st.columns(2)
                    with bp1:
                        if st.button("🔖 À voir", key=f"pt_av_{fid}", use_container_width=True):
                            avoir_ajouter(int(fid), titre, annee)
                            plus_tard_retirer(int(fid)); st.rerun()
                    with bp2:
                        if st.button("🗑 Retirer", key=f"pt_del_{fid}", use_container_width=True):
                            plus_tard_retirer(int(fid)); st.rerun()
