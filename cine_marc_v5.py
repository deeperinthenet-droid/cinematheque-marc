...
def vu_charger():
    data, _ = _gh_read("films_vus.json")
    return data

def vu_sauver(d):
    _, sha = _gh_read("films_vus.json")
    return _gh_write("films_vus.json", d, sha)

def vu_ajouter(fid, titre, annee):
    d, sha = _gh_read("films_vus.json")
    d[str(fid)] = {"titre": titre, "annee": annee, "date": datetime.now().strftime("%d/%m/%Y")}
    _gh_write("films_vus.json", d, sha)
...
def avoir_charger():
    data, _ = _gh_read("films_avoir.json")
    return data

def avoir_sauver(d):
    _, sha = _gh_read("films_avoir.json")
    return _gh_write("films_avoir.json", d, sha)

def avoir_ajouter(fid, titre, annee):
    d, sha = _gh_read("films_avoir.json")
    d[str(fid)] = {"titre": titre, "annee": annee, "date": datetime.now().strftime("%d/%m/%Y")}
    _gh_write("films_avoir.json", d, sha)
...
def filtrer_cache(films, genre_ids=None, provider_ids=None, annee_min=1930, annee_max=2026, note_mini=8.0, tri="vote_average.desc", ok_marcel=False):
    ...
    if lancer:
        with st.spinner("Chargement du catalogue…"):
            if cache_films:
                ...
                brut  = rechercher_films(genre_ids, provider_id, col_key, annee_min, 8.0, tri_val, 1)
                brut += rechercher_films(genre_ids, provider_id, col_key, annee_min, 8.0, tri_val, 2)
