...
def filtrer_cache(films, genre_ids=None, provider_ids=None, annee_min=1930, note_mini=7.5, tri="vote_average.desc"):
...
with tab_browse:
    ...
    if lancer:
        with st.spinner("Chargement du catalogue…"):
            if cache_films:
                ...
                st.session_state.films_ok = resultats[:60]
            else:
    ...
    if st.session_state.film_detail:
        ...
        if "nb_visibles" not in st.session_state or lancer:
            st.session_state.nb_visibles = 60

        films_visibles = films_affiches[:st.session_state.nb_visibles]

        for i in range(0, len(films_visibles), 6):
            cols = st.columns(6, gap="small")
            for j, film in enumerate(films_visibles[i:i+6]):
                with cols[j]:
                    render_card(film, prefix="g")

        # Bouton "Charger plus" si films restants
        restants = len(films_affiches) - st.session_state.nb_visibles
        if restants > 0:
            st.markdown('<div style="text-align:center;padding:2rem 0 1rem;">', unsafe_allow_html=True)
            if st.button(f"▼ Charger 12 de plus ({restants} restants)", key="load_more"):
                st.session_state.nb_visibles += 12
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
